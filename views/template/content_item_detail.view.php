<header class="bg-white rounded-xl shadow overflow-hidden mb-6">
  <div class="relative">
    <img src="<?= htmlspecialchars($content_item->url) ?>" alt="<?= htmlspecialchars($content_item->title) ?>" class="w-full h-56 object-cover" />
    ...
    <div class="absolute bottom-4 left-4 right-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
      <div class="text-white">
        <h1 class="text-lg md:text-2xl font-bold"><?= htmlspecialchars($content_item->title) ?></h1>
        <p class="text-sm opacity-90"><?= htmlspecialchars($content_item->content_type ?? "Plataforma") ?></p>
        <div class="mt-2 flex items-center gap-2 text-sm">
          <span class="inline-block bg-white/20 px-2 py-1 rounded-full">Tags: <?= htmlspecialchars($content_item->content_type ?? "") ?></span>
        </div>
      </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
      <div>
        <form action="/user_item_notes" method="POST" class="space-y-3 w-full">
          ...
          <?php endif; ?>
          <input type="hidden" name="content_item_id" value="<?= htmlspecialchars($content_item->id) ?>" />
